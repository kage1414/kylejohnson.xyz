...
export async function createSnapshot(client: Executor, args: {
  "data": unknown;
}): Promise<{
  "id": string;
}> {
  return client.queryRequiredSingle(`insert Snapshot {
  data := <json>$data
 };`, args);
}
...
export async function getMostRecentSnapshot(client: Executor): Promise<{
  "createdAt": Date;
  "data": unknown;
} | null> {
  return client.querySingle(`select Snapshot {
  createdAt,
  data
}
...
export async function createInvite(client: Executor, args: {
  ...
  return client.querySingle(`insert Invite {
  email := <str>$email,
  key := <str>$key
};
select Invite {
...
export async function deleteInvite(client: Executor, args: {
  "key": string;
}): Promise<{
  "id": string;
} | null> {
  return client.querySingle(`delete Invite
filter .key = <str>$key
limit 1;`, args);
}
...
export async function getInvite(client: Executor, args: {
  ...
  return client.querySingle(`select Invite { id, registered, email }
filter .key = <str>$key
limit 1`, args);
}
...
export async function setRegisteredInvite(client: Executor, args: {
  "key": string;
}): Promise<{
  "id": string;
}[]> {
  return client.query(`update Invite
filter .key = <str>$key
set {
  registered := true
}`, args);
